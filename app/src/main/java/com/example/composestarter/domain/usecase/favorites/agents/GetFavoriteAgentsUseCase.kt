...
class GetFavoriteAgentsUseCase @Inject constructor(
    private val repository: FavoriteAgentsRepository
) {

    suspend  operator fun invoke(): Flow<BaseUIModel<List<FavoriteAgentsEntity>>> {
        return flow {
            emit(Loading())
            emit(
                when (val result = repository.getFavoriteAgents()) {
