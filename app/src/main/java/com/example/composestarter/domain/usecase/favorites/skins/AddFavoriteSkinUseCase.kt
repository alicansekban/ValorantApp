...
class AddFavoriteSkinUseCase @Inject constructor(
    private val repository: FavoriteSkinsRepository
) {
    operator fun invoke(skin: FavoriteSkinsEntity): Flow<BaseUIModel<Any>> {
        return flow {
            emit(Loading())
            emit(
                when (val result = repository.insertFavoriteSkin(skin)) {
